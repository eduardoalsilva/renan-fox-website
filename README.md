# Renan Fox - Artista
Este repositório contém o código-fonte do site pessoal de Renan Fox, um artista que compartilha suas obras e projetos. O site foi desenvolvido para fornecer uma experiência visualmente atraente e interativa aos visitantes, utilizando tecnologias como HTML, CSS e JavaScript.

## Estrutura do Projeto
- `index.html`: O arquivo principal que contém a estrutura do site.
- `assets/css/style.css`: Arquivo de estilos CSS que define a aparência do site.
- `assets/js/script.js`: Arquivo JavaScript que adiciona interatividade ao site.
- `assets/img/`: Diretório que contém todas as imagens utilizadas no site.

## Como Usar
1. Clone o repositório para sua máquina local:
   ```bash
   git clone https://github.com/renan-fox/renan-fox-website.git
   ```
2. Navegue até o diretório do projeto:
   ```bash
   cd renan-fox-website
   ```
3. Abra o arquivo `index.html` em seu navegador preferido para visualizar o site.
4. Para fazer alterações, edite os arquivos CSS e JavaScript conforme necessário.
5. Salve suas alterações e recarregue o navegador para ver as atualizações.

## Licença
Este projeto é de propriedade exclusiva de Renan Fox. A reprodução, distribuição ou modificação do código ou conteúdo sem a devida autorização não é permitida.

## Contato
Para questões relacionadas ao desenvolvimento ou manutenção do site, entre em contato com o desenvolvedor do projeto:

Eduardo Silva
E-mail: eduardoalsilva45@gmail.com

Para informações sobre o trabalho e projetos de Renan Fox, acesse o [site oficial de Renan Fox](https://renanfox.com) ou entre em contato diretamente com ele.

## Sobre o Projeto
Este projeto é uma plataforma digital dedicada à apresentação e divulgação das obras e projetos artísticos de Renan Fox. O objetivo é criar um espaço visualmente atraente e interativo, onde os visitantes possam explorar o universo criativo do artista de forma envolvente e acessível.